...
export default function refeshSeat(agenda) {
    agenda.define("refeshseat", (job, done) => {
        lotrinhModel.find({}, (err, listLotrinh) => {
            if (err) { return done(new Error("Khong the thuc hien task vu nay")); }
            ...
            chuyenxeModel.insertMany(chuyenArr, (e, result) => {
                if (e) { return done(new Error('Loi roi')) }
                console.log("===============================");
                console.log("Tao cong viec thanh cong");
                console.log("===============================");
            })
        });
        done();
