...
const taoChuyenXe = async () => {
  try {
    ...
    listLoTrinh.forEach(async lotrinh => {
      if (4 * lotrinh.thoigianvanchuyen !== lotrinh.xetronglotrinh) {
        return new Error('Xe tham gia trong chuyen khong du de phan cong');
      }
      const listChuyenXe = [];
      const endPoint =
        parseInt((configTuyen.timetoend - configTuyen.timetostart),0);
        if(endPoint===0){
         console.log('===============================');
         console.log('Khong the = o dk');
         console.log('===============================');
         return;
        }
      for (let i = configTuyen.timetostart ; i <= configTuyen.timetostart+endPoint; i++) {
        for (let j = 1; j <= 7; j++) {
          const chuyenDi1 = {
